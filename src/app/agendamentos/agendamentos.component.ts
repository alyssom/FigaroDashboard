  ...
  finalizarServico(agendamento){
    ...
    .subscribe(snapshots => {
      snapshots.forEach(snapshot => {
          if(snapshot.val().nome == agendamento.nome && snapshot.val().dataAgendamento == agendamento.dataAgendamento  && snapshot.val().nomeCliente == agendamento.nomeCliente && snapshot.val().dataAtual == agendamento.dataAtual && snapshot.val().horario == agendamento.horario){
            this.db.list('/agendamentos/', {preserveSnapshot: true})
            .update(snapshot.key, {
              atendido: true,
